...
import {warranteeMe_Detail} from '../../actions/Service';

const height = Dimensions.get('window').height;
const width = Dimensions.get('window').width;

export default class WarranteeMe_Detail extends Component {
  state = {
    title: 'การค้ำประกัน',
    info: {},
  };
  ...
  componentDidMount() {
    ...
    warranteeMe_Detail(id, (res, err) => {
      if (res.data.status == true) {
        this.setState({
          info: res.data.data.guarantee_me_datail[0],
        });
      } else {
      }
    });
